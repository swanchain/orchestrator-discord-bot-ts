...
export class UserService {
    ...
    async transfer(
        ...
        const contract = contractAddress
            ? new web3.eth.Contract(this.commonAbi, contractAddress)
            : null
        if (contract == null) {
        ...
        if (!isAddress(toWalletAddress) || !isAddress(fromWalletAddress)) {
            errorLogger.error(
                `Invalid wallet address. From: ${fromWalletAddress}, To: ${toWalletAddress}`,
            )
            return ''
        }
        ...
        const balance = (await contract.methods
            .balanceOf(fromWalletAddress)
            .call()) as number
        if (balance < parseInt(claimedAmount)) {
            errorLogger.error(
                `Insufficient balance. Balance: ${balance}, Claimed amount: ${claimedAmount}`,
            )
            return ''
        }
        ...
        const gasEstimate = await contract.methods
            .transfer(toWalletAddress, claimedAmount)
            .estimateGas({
        ...
        if (gasCost > walletBalance) {
            errorLogger.error(
                `Insufficient gas. Gas cost: ${gasCost}, Wallet balance: ${walletBalance}`,
            )
            return ''
        }
        ...
        try {
            ...
            // Access tx.hash to get the transaction hash
            const txHash = tx.transactionHash
            const txnReceipt = await web3.eth.getTransactionReceipt(txHash)

            logger.info(
                `-- Transaction successful with transaction hash: ${txHash} at ${new Date().toISOString()}`,
            )
            logger.info(`-- Transaction receipt: ${txnReceipt}`)
            logger.info(
                `-- Gas used: ${
                    txnReceipt['gasUsed']
                } for transaction hash: ${txHash} at ${new Date().toISOString()}`,
            )
            return txHash
        } catch (e) {
    ...
    async recordUserTransaction(
        ...
        logger.info(
            `-- Recording user transaction for ${discordName} at ${new Date().toISOString()}`,
        )
    ...
    async transferAndRecord(
        ...
        if (network === 'OPSWAN') {
            ...
            claimedAmount = web3.utils.toWei(
                Number(claimedAmount).toString(),
                'ether',
            )
        }
        ...
        const isSuccess = await this.recordUserTransaction(
            discordId,
            discordName,
            fromWalletAddress,
            toWalletAddress,
